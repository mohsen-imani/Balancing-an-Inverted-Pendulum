...
double e[STATES][STATES][ACTIONS];
...
int flag; 			     // this flag control the case after the reset 
int tr;
double ep;
...
int action_star (int s_x, int s_y);
/*****************************************************************************/
...
int *explore;
{
 ...
 // trial in the next time step once the pendulum has fallen
  ...
  if (flag == 1) {
     ...
     //a_star = action_star(s_current_x,s_current_y);

     SIGMA = (reward(failure) + GAMMA*Q[s_current_x][s_current_y][a_current] - Q[s_prev_x][s_prev_y][a_prev]);
     e[s_prev_x][s_prev_y][a_prev] += 1;

     int i,j ,k ;
	 for(i = 0; i <= (STATES - 1); i++){
	     for(j = 0; j <= (STATES - 1); j++){
		for(k = 0; k <= (ACTIONS - 1); k++){
		    Q[i][j][k] += ALPHA * SIGMA * e[i][j][k];
