...
if (reset == 1){
  ...
  a_current = (s_current != -1)? get_action_AC(s_current, force, invert, explore): -1;
  //printf("s_prev = %d s_cur = %d a_prev = %d a_curr %d\n",s_prev, s_current,a_prev, a_current);
  
if (flag == 0){ 
  
  delta =  (long double)reward(*fail) + GAMMA * ((s_current != -1)? value[s_current]: value[s_prev]) - value[s_prev];
  //printf("deta = %Lf pref[%d][%d] = %f value[%d] = %f",delta, s_prev, a_prev, pref[s_prev][a_prev], s_prev, value[s_prev]);  

  value[s_prev] = value[s_prev]+ ALPHA*((long double)reward(*fail) + GAMMA * ((s_current != -1)? value[s_current]: value[s_prev]) - value[s_prev]);

  pref[s_prev][a_prev] =  pref[s_prev][a_prev] + BETA * delta;

  //printf(" after pref[%d][%d] = %f value[%d] = %f, pr(%d,0) + pr(%d, 1) = %f + %f = %f \n", s_prev, a_prev, pref[s_prev][a_prev], s_prev ,value[s_prev],s_prev, s_prev,pr(s_prev,0),pr(s_prev,1), pr(s_prev,0) + pr(s_prev,1));  
  //printf("updated Q [%d][%d]= %Lf \n",s_prev,a_prev,Q[s_prev][a_prev]);
  }
